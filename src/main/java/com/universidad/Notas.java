                ...
                if (compare.contains(toUper)) {
                    ...
                    for (int y = 0; y < BdEstudiantes.listarEstudiantes().size(); y++) {
                        ...
                        if (y == i) {
                            ...
                            int cantNotas = Integer.parseInt(JOptionPane.showInputDialog("Cantidad de notas a ingresar al alumno" + BdEstudiantes.listarEstudiantes().get(y).nombre + " " + BdEstudiantes.listarEstudiantes().get(y).apellido + " " + "en: " +  BdEstudiantes.listarEstudiantes().get(y).getIdMateria()));

                            for (int j = 0; j < cantNotas; j++) {
                                int countN = 1;
                                notas = Integer.parseInt(JOptionPane.showInputDialog("Ingresar nota:" + countN + "a estudiante: " + BdEstudiantes.listarEstudiantes().get(y).nombre + " " + BdEstudiantes.listarEstudiantes().get(y).apellido));
